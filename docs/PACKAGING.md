...
|---|---|---|
| GitHub Releases | `mangokingTW/HdrScreenshotSyncer` | `release.yml` on tag push |
| Microsoft Store | Store ID `9NS238QVJ9CS` | `release.yml` (stable tags only) |
| Chocolatey | `hdrscreenshotsyncer` | `chocolatey.yml` on release published |
...
1. **Bump the version in three places** (they must agree):
   - `CMakeLists.txt` — `project(HdrScreenshotSyncer VERSION X.Y.Z ...)` (the
...
6. **Create the GitHub release** — notes from the CHANGELOG section plus a fixed
   install/verify footer (or auto-generated notes if the tag has no section).
...
- **Store "What's new"** = the highlights **only**. The workflow splits the
  section at the `繁體中文:` marker into `storenotes.en.md` / `storenotes.zh.md`
  and assigns per Store listing by language (`en-us` → English, `zh-tw` →
  Chinese). This app currently has a **single `en-us` listing**, so it receives
  the **full bilingual** note; add a `zh-tw` listing in Partner Center and the
  split kicks in automatically.
...
The Traditional-Chinese listing copy (Title + Description) to paste when adding
the `zh-tw` listing lives in
[`packaging/store/listing.zh-TW.md`](../packaging/store/listing.zh-TW.md).

## Microsoft Store

MSIX identity: `Name="MangoYen.HDRScreenshotSyncer"`,
...
produced by CI is unsigned. This is a full-trust packaged desktop app
(`rescap:runFullTrust`) that needs no elevation, so the Store build is fully
functional.
...
- **Secrets** (repo → Settings → Secrets): `PARTNER_CENTER_TENANT_ID`,
  ...
  (One Entra app + Manager role serves this app and its sibling ImeModePersistence
  — only the Store ID differs.)
